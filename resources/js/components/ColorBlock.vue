<template>
  <div
    ...
    :style="{ 'background-color': colorObject.toString('hex') }"
  ...
  >
    ...
    >
      ...
      <svg
        class="h-6 w-6 m-auto my-3"
        v-on:click="copyToClipboard(colorObject.toHexString())"
        v-bind:class="{
          'hover:text-white': isLight,
          'hover:text-black': !isLight,
        }"
      ...
      <div
        v-on:click="copyToClipboard(colorObject.toString())"
    ...
    <div>
      <div class="text-center my-2">
        {{ colorObject.toHexString().toUpperCase() }}
      </div>
      <div class="text-center text-sm font-normal my-2">
        {{
          namer(colorObject.toString(), { pick: ["ntc"] })[
            "ntc"
          ][0].name.toUpperCase()
        }}
      </div>
...
export default {
  mounted() {
    ...
    this.colorObject = new tinycolor(this.color);
    this.isLight = this.colorObject.isLight();
  },
  props: ["color"],
  data() {
    return {
